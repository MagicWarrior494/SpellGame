...
namespace Vulkan {
    ...
    inline DescriptorResult CreateDescriptors(VulkanCore* VC, const DescriptorSetInfo& info)
    {
        ...
        for (const auto& b : info.bindings) {
            uint32_t perSetCount = !b.images.empty() ? static_cast<uint32_t>(b.images.size() / framesInFlight)
                : !b.buffers.empty() ? static_cast<uint32_t>(b.buffers.size() / framesInFlight)
                : b.count;
