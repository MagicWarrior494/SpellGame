...
namespace Vulkan {
	...
	PhysicalDevice GetPhysicalDeviceProperties(VkPhysicalDevice physicalDevice)
	{
	...
	void CreateLogicalDevice(VulkanCore* VC)
	{
		...
		vkGetPhysicalDeviceQueueFamilyProperties(vulkanCore.physicalDeviceData.vkPhysicalDevice, &queueFamilyPropertyCount, nullptr);

		queueFamilyProperties.resize(queueFamilyPropertyCount);

		vkGetPhysicalDeviceQueueFamilyProperties(vulkanCore.physicalDeviceData.vkPhysicalDevice, &queueFamilyPropertyCount, queueFamilyProperties.data());

		bool graphicsQueue = false;
		bool presentQueue = false;

		PhysicalDevice QFI = GetPhysicalDeviceProperties(vulkanCore.physicalDeviceData.vkPhysicalDevice);
		...
		if (vkCreateDevice(vulkanCore.physicalDeviceData.vkPhysicalDevice, &deviceCreateInfo, nullptr, &vulkanCore.vkDevice) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create Logical Device!");
		}

		vkGetDeviceQueue(vulkanCore.vkDevice, vulkanCore.physicalDeviceData.graphicsIndex.value(), 0, &vulkanCore.vkGraphicsQueue);
		vkGetDeviceQueue(vulkanCore.vkDevice, vulkanCore.physicalDeviceData.presentIndex.value(), 0, &vulkanCore.vkPresentQueue);
	}
}